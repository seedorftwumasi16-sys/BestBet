      ...
      <AnimatePresence>
        ...
        {selections.map((sel) => (
          ...
          >
            ...
            {sel.market === "Correct Score" ? (
              <>
                <p className="text-sm font-semibold mt-0.5 truncate">Correct Score: {sel.selection}</p>
                <p className="text-xs text-bestbet-gray-muted mt-1">
                  Odds: <span className="font-bold text-bestbet-yellow tabular-nums">{formatOdds(sel.odds)}</span>
                </p>
              </>
            ) : (
              <>
